...
function updateLayoutFromSettings() {
  ...
  if (isListActive) {
    ...
    const listLayoutBtn = document.querySelector(
      '.tf-view-layout-switch[data-value-layout="list"]'
    );
...
function initProductFilters() {
  ...
  document.querySelectorAll('input[name="availability"]').forEach((input) => {
    input.addEventListener("change", function () {
      ...
      const checkedAvailability = Array.from(
      ...
      filters.availability =
        checkedAvailability.length > 0 ? checkedAvailability[0] : null;
...
function resetAllFilters() {
  ...
  document
    ...
    .forEach((input) => (input.checked = false));
  document
    .querySelectorAll(".size-check, .color-check")
    .forEach((btn) => btn.classList.remove("active"));
...
function fetchFilteredProducts(filterUrl) {
  ...
  if (window.fetch) {
    ...
    xhr.onerror = function () {
      handleFilterError(
        new Error(
          window.translations?.shop?.network_error || "Network error occurred"
        ),
        filterUrl
      );
      showFilterLoadingState(false);
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      if (newGridProducts && currentGridLayout) {
        ...
        const currentClasses = currentGridLayout.className.replace(
...
function updatePaginationFromResponse(newGridProducts, newListProducts) {
  ...
  if (newGridProducts) {
    ...
    const currentGridPagination = document.querySelector(
      "#gridLayout .wg-pagination"
    );
    ...
    if (newGridPagination && currentGridPagination) {
      ...
      document
        ...
        .appendChild(newGridPagination.cloneNode(true));
    }
  }
  ...
  if (newListProducts) {
    ...
    const currentListPagination = document.querySelector(
      "#listLayout .wg-pagination"
    );
    ...
    if (newListPagination && currentListPagination) {
      ...
      document
        ...
        .appendChild(newListPagination.cloneNode(true));
    }
  }
}
...
function getTotalProductsFromResponse(doc) {
  ...
  if (collectionSection) {
    const totalProducts = parseInt(
      collectionSection.getAttribute("data-total-products")
    );
  ...
  if (fallbackElement) {
    const totalProducts = parseInt(
      fallbackElement.getAttribute("data-total-products")
    );
    return isNaN(totalProducts) ? 0 : totalProducts;
  }
  return 0;
  ...
  countElements.forEach((el) => {
    if (
      el.textContent.includes(
        window.translations?.shop?.products_found || "Products found"
      ) ||
  ...
  const noProductsMessage = `
      <div class="no-products-found">
          <div class="no-products-content">
              <h3>${
                window.translations?.shop?.no_products_found ||
                "No products found"
              }</h3>
              <p>${
              ...
              <button class="btn-reset-filters" onclick="resetAllFilters()">${
                window.translations?.shop?.reset_all_filters ||
                "Reset All Filters"
              }</button>
          </div>
      </div>
...
function initializeFiltersFromURL() {
  ...
  const sizeFilter =
    urlParams.get("filter.v.option1") || urlParams.get("filter.v.option2");
  if (sizeFilter) {
  ...
  if (priceSlider) {
    ...
    if (filters.minPrice > minPrice || filters.maxPrice < maxPrice) {
      ...
      const displayMaxPrice =
        typeof filters.maxPrice === "number" ? filters.maxPrice : 500;
      tags.push(
...
function updateLastVisibleItem() {
  setTimeout(() => {
    document
      .querySelectorAll(".card-product.style-list")
      .forEach((item) => item.classList.remove("last"));
    const lastVisible = Array.from(
      document.querySelectorAll(".card-product.style-list")
    )
...
function initSortFunctionality() {
  ...
  isListActive = document
    .querySelector(".sw-layout-list")
    ?.classList.contains("active");
  let originalProductsList = Array.from(
    document.querySelectorAll("#listLayout .card-product")
  ).map((p) => p.cloneNode(true));
  let originalProductsGrid = Array.from(
    document.querySelectorAll("#gridLayout .card-product")
  ).map((p) => p.cloneNode(true));
  let paginationList = document
    .querySelector("#listLayout .wg-pagination")
    ?.cloneNode(true);
  let paginationGrid = document
    .querySelector("#gridLayout .wg-pagination")
    ?.cloneNode(true);
...
function applySorting(
  ...
  if (sortValue === "best-selling") {
    ...
    const originalProducts = isListActive
      ? originalProductsList
      : originalProductsGrid;
...
function initLayoutSwitching() {
  ...
  function updateLayoutDisplay() {
    ...
    if (isListActive) {
      ...
      if (listLayout) listLayout.style.display = "";
      document
        .querySelector(".wrapper-control-shop")
        ?.classList.add("listLayout-wrapper");
      document
        .querySelector(".wrapper-control-shop")
        ?.classList.remove("gridLayout-wrapper");
      return;
    }
    ...
    const selectedLayout =
      userSelectedLayout ||
      (window.collectionData && window.collectionData.defaultGridLayout);
    ...
    if (selectedLayout) {
      if (windowWidth <= 767) {
        ...
        windowWidth <= 1400 &&
        ["tf-col-5", "tf-col-6", "tf-col-7"].includes(selectedLayout)
      ) {
    ...
    if (windowWidth <= 767) {
      ...
      document
        .querySelector(".wrapper-control-shop")
        ?.classList.add("gridLayout-wrapper");
      document
        .querySelector(".wrapper-control-shop")
        ?.classList.remove("listLayout-wrapper");
    }
  }
    ...
    if (window.collectionData && window.collectionData.defaultGridLayout) {
      ...
      const defaultGridBtn =
        document.querySelector(
          '.tf-view-layout-switch[data-value-layout="tf-col-3"]'
        ) ||
        document.querySelector(
          '.tf-view-layout-switch[data-value-layout="tf-col-4"]'
        );
      if (defaultGridBtn) {
...
function initLoadMore() {
  ...
  function showMoreItems(layout, itemsPerPage, itemsDisplayed) {
    const hiddenItems = layout.querySelectorAll(
  ...
  function checkLoadMoreButton(layout) {
    const hiddenCount = layout.querySelectorAll(
      '.card-product[style*="display: none"]'
    ).length;
...
function handleLoadMore(layout) {
  ...
  button.querySelector(".text").textContent =
    window.translations?.shop?.loading || "Loading...";
  ...
  setTimeout(() => {
    ...
    if (endIndex >= totalProducts) {
      ...
      button.querySelector(".text").textContent =
        window.translations?.shop?.load_more || "Load more";
    }
...
function enforceMobileLayout() {
  if (window.innerWidth <= 767) {
    ...
    if (gridLayout && !isListActive) {
      ...
      gridLayout.className = gridLayout.className.replace(
        /tf-col-\d+/,
        "tf-col-2"
      );
...
function hasActiveFilters() {
  ...
  if (priceSlider && priceSlider.noUiSlider) {
    ...
    const maxPrice = parseInt(priceSlider.dataset.max, 10) || 500;
    if (
      parseInt(currentValues[0]) > minPrice ||
      parseInt(currentValues[1]) < maxPrice
    ) {
      return true;
    }
  } else if (priceSlider) {
  ...
  const hasActiveAvailability =
    document.querySelector('input[name="availability"]:checked') !== null;
  const hasActiveBrand =
    document.querySelector('input[name="brand"]:checked') !== null;
  const hasActiveSale =
    document.querySelector(".shop-sale-text.active") !== null;
